...
def main():
    ...
    for category_name, symbols in categories.items():
        ...
        # prepare output file name for this category
            ...
            try:
                for tf in REQUEST_TFS:
                    for attempt in range(RETRY_COUNT):
                        ...
                        # couldn't fetch this timeframe after retries
                    ...
                    sros_val = True if stoch_df['sros'].iloc[-1] == 1 else False
                    srob_val = True if stoch_df['srob'].iloc[-1] == 1 else False
                    srd_val  = True if stoch_df['srd'].iloc[-1] != 0 else False
                    ...
                    results[symbol][tf] = {
                        ...
                        "sros_fmt": format_cell(sros_val),
                        "srob_fmt": format_cell(srob_val),
                        "srd_fmt": format_cell(srd_val),
                    }
                ...
                # -----------------------
                # این خط باعث می‌شود بعد از پردازش هر کوین، خروجی HTML فعلی ذخیره شود.
                try:
                    save_html(results, REQUEST_TFS, file_name)
                    print(f"💾 خروجی موقت ذخیره شد: {file_name} (تا نماد {symbol})")
                except Exception as e_save:
                    print(f"⚠️ خطا هنگام ذخیره‌سازی {file_name}: {e_save}")
                # -----------------------
        ...
        # once the category loop is done we already have the final file saved (since we save after each symbol)
        print(f"✅ فایل نهایی {file_name} ساخته/به‌روز شد.")
